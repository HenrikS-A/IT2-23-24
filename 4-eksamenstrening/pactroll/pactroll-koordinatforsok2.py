...
class Matbit(Spillobjekt):
    ...
    def opprett_nytt_koordinat(cls, koordinatliste: list[tuple]) -> list[int]:
       while True:
            ...
            # Sjekker om det nye punktet overlapper med punktene som er i koordinatlista
            overlapp = False
            for koord in koordinatliste:
                if abs(koord[0] - x) <= 65 or abs(koord[1] - y) <= 65: # Sidelengden er 60 – plusser på litt margin
                    overlapp = True
                    break
