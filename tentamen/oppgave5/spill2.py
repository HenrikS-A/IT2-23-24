...
class Ball(Spillobjekt):
    ...
    def sjekk_flere_baller(self, murliste):
        if len(murliste) % 10 == 0:
            return True
        return False
...
while True:
    ...
    for ball in baller:
        ...
        if kollisjon:
            ...
            oeke_baller = ball.sjekk_flere_baller(mur)
            if oeke_baller:
                gammel_ball_pos = ball.rect.center
                ny_ball = Ball()
                ny_ball.rect.center = gammel_ball_pos
                ny_ball.x_fart = ny_ball.ballfart
